...
function LeftNavItem({
  ...
  enableSelectedItemCustomStyling,
}) {
  ...
  const selectedStyle = enableSelectedItemCustomStyling && isSelected ? {
    backgroundColor: selectedItemBgColor || "#E2001A",
    color: selectedItemTextColor || "#ffffff",
  } : {};
  ...
  return (
    <li className="left-nav__item">
      <button
        ...
        style={{ paddingLeft: `${12 + level * 12}px`, ...selectedStyle }}
        onClick={() => onSelect(item, { canToggleMainMenu })}
      >
        <span
          className="left-nav__icon"
          aria-hidden="true"
          style={enableSelectedItemCustomStyling && isSelected ? { color: selectedItemTextColor || "#ffffff" } : {}}
        >
          {item.icon}
        </span>
          {!collapsed && (
        ...
        <ul className="left-nav__list">
          {item.children.map((child) => (
            <LeftNavItem
              ...
              enableSelectedItemCustomStyling={enableSelectedItemCustomStyling}
            />
          ))}
        </ul>
      )}
...
export default function LeftNav({
  ...
  selectedItemBgColor = "#E2001A",
  selectedItemTextColor = "#ffffff",
  enableSelectedItemCustomStyling = false,
}) {
  ...
  const selectedParentMainMenuId = useMemo(() => {
    const parent = (items || []).find(
      (item) => Array.isArray(item.children) && item.children.some((child) => child.id === selectedId)
    );
  ...
  const [expandedMainMenuIds, setExpandedMainMenuIds] = useState(() => {
    ...
    if (selectedParentMainMenuId) {
      return new Set([selectedParentMainMenuId]);
    }
    return new Set();
  });
  ...
  useEffect(() => {
    if (!enableMainMenuCollapse || !selectedParentMainMenuId) {
      return;
    }

    setExpandedMainMenuIds((prev) => {
      if (prev.has(selectedParentMainMenuId)) {
        return prev;
      }
      const next = new Set(prev);
      next.add(selectedParentMainMenuId);
      return next;
    });
  }, [enableMainMenuCollapse, selectedParentMainMenuId]);
