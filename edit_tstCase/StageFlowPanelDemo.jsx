...
const TestGenStageForm = forwardRef(function TestGenStageForm(props, ref) {
  return (
    <TestGenFormPanel
      ...
      squashConfig={{
        getFolderService: (folderId, options) => getSquashTestCaseFolder(folderId, options),
        createFolderService: ({ name, description, parentId }, options) =>
          createSquashTestCaseFolder({ name, description, parentId }, options),
        saveTestCasesService: (payload, options) => saveTestCasesToSquash(payload, options),
      }}
...
export default function StageFlowPanelDemo({
  ...
  useEffect(() => {
    ...
    createCaseId()
      ...
      .catch((error) => {
        if (!active) return;
        const msg = error?.response?.data?.message ?? error?.message ?? "Failed to initialize case.";
  ...
  if (!showCaseIdStage && bootstrapLoading) {
    return <div className="sfp__submit-warning" role="status">Creating Case ID...</div>;
  }

  if (!showCaseIdStage && bootstrapError) {
    return <div className="sfp__form-error" role="alert">Error: {bootstrapError}</div>;
  }
