...
import {
  ...
  fetchTestCases,
  exportTestCases,
  getSquashTestCaseFolder,
  createSquashTestCaseFolder,
  saveTestCasesToSquash,
  saveStageSnapshot
} from "../../services/testManagementService";

const SQUASH_CONFIG = {
  getFolderService: getSquashTestCaseFolder,
  createFolderService: createSquashTestCaseFolder,
  saveTestCasesService: saveTestCasesToSquash,
};
...
function mapTestGenData(raw, taskId) {
  ...
  return { taskId, testcases, rawTestGenerationJson: raw };
}
...
async function getTaskIdService({ "case-id": caseId }) {
  if (!caseId) {
    throw new Error("Case ID is not initialized.");
  }
...
const ValidationStageForm = forwardRef(function ValidationStageForm(props, ref) {
  return (
    <ValidationFormPanel
      ...
      {...props}
      title="Validation"
      closeCaseService={(taskId) => approveOrReject(taskId, "reject")}
    />
  );
});
...
const TestGenStageForm = forwardRef(function TestGenStageForm(props, ref) {
  return (
    <TestGenFormPanel
      ...
      exportService={exportTestCases}
      squashConfig={SQUASH_CONFIG}
      testGenerationEditorMode={props.testGenerationEditorMode}
    />
  );
});
...
const DEFAULT_SHOW_CASE_ID_STAGE = isTruthyFlag(import.meta.env.VITE_SHOW_CASE_ID_STAGE);
...
function extractBackendErrorMessage(error, fallback) {
  ...
  if (data && typeof data === "object") {
    const candidate =
      ...
      data.error ??
      data.detail ??
      (typeof data.trace === "string" ? data.trace.split("\n")[0] : null);
    ...
    try {
      ...
      // ignore serialization errors and continue to generic fallback
    }
  }
...
export default function StageFlowPanelDemo({
  ...
  showCaseIdStage     = DEFAULT_SHOW_CASE_ID_STAGE,
}) {
  ...
  const [bootstrapLoading, setBootstrapLoading] = useState(mode === "create" && !showCaseIdStage);
  const [bootstrapError, setBootstrapError] = useState("");

  const resolveCaseId = useCallback(
    (context) => context?.["case-id"] ?? activeCaseIdRef.current ?? initialCaseId ?? null,
    [initialCaseId]
  );

  const stagesWithProps = useMemo(
    () => {
      const visibleStages = showCaseIdStage
        ? STAGES
        : STAGES.filter((stage) => stage.id !== "case-id");

      return visibleStages.map((stage) => {
        const next = { ...stage };

        if (stage.id === "upload-doc" || stage.id === "approval") {
          next.service = async (context) => getTaskIdService({ "case-id": resolveCaseId(context) });
        }

        if (stage.id === "validation") {
          next.service = async (context) => fetchVariablesService({ "case-id": resolveCaseId(context) });
        }

        if (stage.id === "test-gen") {
          next.service = async (context) => fetchTestCasesService({ "case-id": resolveCaseId(context) });
          next.componentProps = {
            ...(stage.componentProps || {}),
            testGenerationEditorMode,
            allowEditInReadOnlyModes: true,
          };
        }
    ...
    [resolveCaseId, showCaseIdStage, testGenerationEditorMode]
  );

  useEffect(() => {
    let active = true;

    if (mode !== "create" || showCaseIdStage) {
    ...
    createCaseId()
      .then(async (caseId) => {
        ...
        completedUpToRef.current = 0;
        stageDataRef.current = { "case-id": caseId };
        await saveSnapshotWithRetry(caseId, "case-id", caseId, 0);
      })
      .catch((error) => {
        if (!active) return;
        const msg = extractBackendErrorMessage(error, "Failed to initialize case.");
  ...
  }, [mode, showCaseIdStage]);

  const handleStageComplete = useCallback(
    (stageId, serviceData, stageIndex, completedStatus) => {
      if (showCaseIdStage && stageId === "case-id" && serviceData) {
      ...
      const persistedStageIndex = showCaseIdStage ? stageIndex : stageIndex + 1;
      if (persistedStageIndex > completedUpToRef.current) completedUpToRef.current = persistedStageIndex;

      saveSnapshotWithRetry(caseId, stageId, snapshotPayload, completedUpToRef.current)
        .then(() => onSaved?.())
        .catch(console.error);
    },
      [onSaved, showCaseIdStage]
  );
  ...
  // create mode (default)
  if (!showCaseIdStage && bootstrapLoading) {
    return <div className="fp__status fp__status--neutral" role="status">Creating Case ID...</div>;
  }

  if (!showCaseIdStage && bootstrapError) {
    return <div className="fp__status fp__status--error" role="alert">Error: {bootstrapError}</div>;
  }
