...
export default function InlineFullscreenPanel({
  ...
  useEffect(() => {
    if (!isOpen || (!isFullscreen && !overlay)) return undefined;
  ...
  const panelClasses = [
    "ifs-panel",
    overlay ? "ifs-panel--overlay" : "",
    transparent ? "ifs-panel--transparent" : "",
    isFullscreen ? "ifs-panel--fullscreen" : "",
    className,
  ].filter(Boolean).join(" ");
  ...
  const panel = (
    ...
    >
      <header className={`ifs-panel__header${compact ? " ifs-panel__header--compact" : ""}`}>
        <div className="ifs-panel__header-left">
          <button
            type="button"
  ...
  if (overlay) {
    return (
      <>
        <div className="ifs-panel__overlay-backdrop" onClick={onBack} aria-hidden="true" />
        {panel}
      </>
    );
  }
