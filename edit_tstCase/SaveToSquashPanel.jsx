...
const DEFAULT_MAP_TEST_CASES = (tableTestCases, targetFolderId) => {
  return (Array.isArray(tableTestCases) ? tableTestCases : []).map((tc) => ({
    _type: "test-case",
    name: String(tc.testCaseId || "").trim() || "Untitled Test Case",
    parent: { id: targetFolderId, _type: "test-case-folder" },
    prerequisite: toParagraphs(tc.preconditions),
    description: String(tc.testDescription || "").trim(),
    automated_test_technology: "Playwright",
    steps: (Array.isArray(tc.testStepAction) ? tc.testStepAction : [])
      .map((step) => String(step || "").trim())
      .filter(Boolean)
      .map((step) => ({
      _type: "action-step",
      action: `<p>${escapeHtml(step)}</p>`,
      expected_result: "<p>Success</p>",
    })),
  }));
};
...
export default function SaveToSquashPanel({
  ...
  const setEnableTableEditing = (nextEnabled) => {
    onEnableTableEditingChange?.(nextEnabled);
  };
  ...
  const [squashApiToken, setSquashApiToken] = useState("");
  ...
  const trimmedSquashApiToken = squashApiToken.trim();
  const squashRequestOptions = useMemo(
    () =>
      trimmedSquashApiToken
        ? {
            apiToken: trimmedSquashApiToken,
          }
        : {},
    [trimmedSquashApiToken]
  );
  ...
  useEffect(() => {
    ...
    const timerId = setTimeout(() => {
      squashConfig
        .getFolderService(Number(idText), squashRequestOptions)
        .then((folder) => {
  ...
  }, [existingFolderId, existingFolder?.id, mode, panelOpen, squashConfig, squashRequestOptions]);
  ...
  }, [parentFolderId, parentFolder?.id, mode, panelOpen, squashConfig, squashRequestOptions]);
  ...
  const handleSaveToSquash = async () => {
    ...
    if (!Array.isArray(tableTestCases)) {
      setStatus({
        type: "error",
        message: "Test generation table data is not available yet.",
      });
      return;
    }
    ...
    try {
      ...
      const payload = mapPayload(tableTestCases, targetFolderId);
    ...
    <div className="fp__squash-wrap">
      {panelOpen && (
        <div className="fp__squash-panel" role="group" aria-label="Save to Squash panel">
          ...
          {mode === "existing" ? (
            <div className="fp__squash-fields fp__squash-fields--existing">
              ...
              <div className="fp__squash-field">
                <div className="fp__field-block fp__field-block--compact">
                  <label className="fp__field-label" htmlFor="fp-squash-api-token">Squash API token (optional)</label>
                  <input
                    id="fp-squash-api-token"
                    type="password"
                    className="fp__input fp__squash-input"
                    placeholder="Enter API token to override default"
                    value={squashApiToken}
                    onChange={(e) => {
                      setSquashApiToken(e.target.value);
                      setExistingFolder(null);
                      setParentFolder(null);
                    ...
                    disabled={showModeBusy}
                    aria-label="Squash API token optional"
                  />
                </div>
              </div>
            </div>
          ...
          <label className="fp__squash-radio fp__squash-radio--checkbox">
            <input
              type="checkbox"
              checked={enableTableEditing}
              onChange={(e) => setEnableTableEditing(e.target.checked)}
              disabled={showModeBusy}
            />
            <span>Enable data table editing</span>
          </label>
      ...
      <div className="fp__squash-actions">
        <button
          type="button"
          className={`fp__btn ${panelOpen ? "fp__btn--primary" : "fp__btn--grey fp__btn--accent-hover"}`}
        ...
        {panelOpen && (
          <button
            type="button"
            className="fp__btn fp__btn--ghost fp__btn--accent-hover"
            onClick={handleClose}
            disabled={saving}
          >
            <CloseIcon />
