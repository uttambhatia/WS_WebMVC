...
const EMPTY_TESTCASES = Object.freeze([]);

const stripRowId = ({ __rowId, ...rest }) => rest;

const rowsContentEqual = (left, right) => {
  if (left === right) return true;
  if (!Array.isArray(left) || !Array.isArray(right)) return false;
  if (left.length !== right.length) return false;
  for (let i = 0; i < left.length; i += 1) {
    if (JSON.stringify(stripRowId(left[i])) !== JSON.stringify(stripRowId(right[i]))) {
      return false;
    }
  }
  return true;
...
const TestGenFormPanel = forwardRef(function TestGenFormPanel(
  ...
  const normalizedRowsFromService = useMemo(() => normalizeRows(testcases), [testcases]);

  useEffect(() => {
    setLocalRows((previousRows) => (
      rowsContentEqual(previousRows, normalizedRowsFromService)
        ? previousRows
        : normalizedRowsFromService
    ));
  }, [normalizedRowsFromService]);

  const tableRowsForSquash = useMemo(
    () => localRows.map(stripRowId),
    [localRows]
  );
      ...
      {isLoading ? (
        ...
        <div className="fp__export-bar">
          {!exportOpen && (
            <SaveToSquashPanel
              taskId={taskId}
              tableTestCases={tableRowsForSquash}
          ...
          {!squashOpen && (
            <div className="fp__export-actions">
              <button
                type="button"
                className={`fp__btn ${exportOpen ? "fp__btn--primary" : "fp__btn--grey fp__btn--accent-hover"}`}
