 ...
 *   buildServerQuery   {fn(baseParams) => any}               Optional adapter for backend query shape.
 *   reloadToken        {any}                                  Change value to force server-side reload.
 *   filterMode         {"instant"|"manual"}                 Filter application mode (default: "instant").
 *   manualFilterDelayMs {number}                              Debounce delay (ms) for auto-applying manual filters while typing.
 *                                                             Defaults: 150 (local) and 450 (server-side).
 *   editableMode       {"local"|"server"}                   Editable behavior mode (default: "local").
 *   rowActions         {fn(row) => Array<Action>}            Optional per-row custom action descriptors.
 *   onRowAction        {fn(action, row)}                      Optional handler for custom row action clicks.
 *   renderRowActions   {fn({row, actions, onAction}) => JSX} Optional custom renderer for row actions cell.
 *   rowActionsColumnLabel {string}                            Custom actions column label (default: "Actions").
 *   rowActionsColumnWidth {string}                            Custom actions column width (default: "16%").
 *   downloadExcel      {boolean}                              Show Excel download button.
 *   toolbarExtra       {ReactNode}                            Extra content appended inside the toolbar row (e.g. a Refresh button).
...
function ListPlusIcon({ className }) {
  const svgClassName = `lucide lucide-list-plus-icon lucide-list-plus${className ? ` ${className}` : ""}`;
  return (
    <svg
...
function PencilIcon({ className }) {
  const svgClassName = `lucide lucide-pencil-icon lucide-pencil${className ? ` ${className}` : ""}`;
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="24"
...
function ListMinusIcon({ className }) {
  const svgClassName = `lucide lucide-list-minus-icon lucide-list-minus${className ? ` ${className}` : ""}`;
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="24"
...
function SaveIcon({ className }) {
  const svgClassName = `lucide lucide-save-icon lucide-save${className ? ` ${className}` : ""}`;
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="24"
...
function rowsDiffer(columns, previousRow, nextRow) {
  return columns.some((col) => {
    const prevValue = previousRow?.[col.key];
    const nextValue = nextRow?.[col.key];
...
export default function DataTable({
  ...
  const editableEnabled = editable && (editableMode === "server" || !serverSide);
  const activeFilters = filterMode === "manual" ? appliedFilters : filters;
  const hasCustomRowActions = typeof rowActions === "function" || typeof renderRowActions === "function";
  const {
    rowKey = "id",
    ...
    createEmptyRow,
    confirmRemoveMessage = "Are you sure you want to remove the record? This action cannot be undone.",
    onEditRow,
    onSaveRow,
    onRemoveRow,
    onRowsChange,
  ...
  const resolvedManualFilterDelayMs =
    typeof manualFilterDelayMs === "number"
      ? manualFilterDelayMs
      : (serverSide ? 450 : 150);
  ...
  const fetchServerData = useCallback(async () => {
    ...
    try {
      const baseParams = { page, pageSize, sort, filters: activeFilters };
      const params = typeof buildServerQuery === "function" ? buildServerQuery(baseParams) : baseParams;
  ...
  }, [serverSide, service, buildServerQuery, page, pageSize, sort, activeFilters, reloadToken]);

  useEffect(() => {
    if (serverSide) fetchServerData();
  }, [fetchServerData, serverSide, reloadToken]);

  const sourceRows = serverSide ? serverData : data;
  ...
  useEffect(() => {
    ...
    setEditableRows((prev) => {
      ...
      const syncedRows = createInternalRows(sourceRows, rowKey).map((item) => {
        const existingRow = previousRowsByKey.get(String(item.row?.[rowKey] ?? ""));
        if (!existingRow) {
          return item;
        }
        ...
        return {
          ...item,
          isAdded: Boolean(existingRow.isAdded),
          isEdited: Boolean(existingRow.isEdited),
        };
      });

      return [...pendingNewRows, ...syncedRows];
    });
  }, [sourceRows, editableEnabled, rowKey]);

  /* Reset page when filters/sort/pageSize change (after initial mount) */
  useEffect(() => {
      ...
      commitEditableRows((prev) => (
        prev.map((current) => (
          current.internalId === item.internalId
            ? (() => {
                const isEditedNow = !current.isNew && rowsDiffer(columns, current.row, nextRow);
                return {
                  ...current,
                  row: { ...nextRow },
                  draft: { ...nextRow },
                  isEditing: false,
                  isNew: false,
                  isAdded: current.isAdded || current.isNew,
                  isEdited: current.isEdited || isEditedNow,
                  errors: {},
                  actionError: "",
                  pendingAction: "",
                };
  ...
  const openRemoveConfirm = useCallback((item) => {
  ...
  const performRemove = useCallback(async (item) => {
    if (!item) {
      return;
    }
  ...
  }, [closeRemoveConfirm, performRemove, removeConfirmItem]);
  ...
  const clientSource = useMemo(() => {
    ...
    return sourceRows;
  }, [sourceRows, editableEnabled, editableRows]);

  const clientFiltered = useMemo(() => {
    if (serverSide) return clientSource;
    if (!editableEnabled) {
      return applyClientFilters(clientSource, activeFilters);
    }
    return filterEditableRows(clientSource, activeFilters);
  }, [serverSide, editableEnabled, clientSource, activeFilters]);
  ...
  useEffect(() => {
    ...
    if (resolvedManualFilterDelayMs <= 0) {
    ...
    const timerId = window.setTimeout(() => {
      setAppliedFilters(filters);
    }, resolvedManualFilterDelayMs);

    return () => {
      window.clearTimeout(timerId);
    };
  }, [filterMode, resolvedManualFilterDelayMs, filters]);
  ...
  }, [filters, sort, page, pageSize, onStateChange]);
  ...
  const displayColumns = useMemo(() => (
    [
      ...
      ...columns,
      ...(hasCustomRowActions ? [{ key: "__dt_row_actions", label: rowActionsColumnLabel, width: rowActionsColumnWidth, editable: false }] : []),
  ...
  const renderActionCell = (item) => {
    ...
    return (
      <div className="dt__row-actions">
        {item.isEditing ? (
          <>
            <button type="button" className="dt__action-btn dt__save-btn" onClick={() => handleSave(item)} disabled={busy}>
              <SaveIcon className="dt__save-icon" />
              {busy && item.pendingAction === "save" ? "Saving..." : saveActionLabel}
            </button>
            <button type="button" className="dt__action-btn dt__edit-btn" onClick={() => handleRemove(item)} disabled={busy}>
              <ListMinusIcon className="dt__edit-icon" />
              {busy && item.pendingAction === "remove" ? "Removing..." : removeActionLabel}
            </button>
          </>
        ) : (
          <>
            <button type="button" className="dt__action-btn dt__edit-btn" onClick={() => handleEdit(item)} disabled={busy}>
              <PencilIcon className="dt__edit-icon" />
  ...
  const resolveActionClassName = (variant) => {
    if (variant === "primary") return "dt__action-btn dt__action-btn--primary";
    if (variant === "danger") return "dt__action-btn dt__action-btn--danger";
    if (variant === "ghost") return "dt__action-btn dt__action-btn--ghost";
    return "dt__action-btn";
  };

  const renderCustomActionCell = (displayRow) => {
    const actions = typeof rowActions === "function" ? (rowActions(displayRow) || []) : [];

    const runAction = (action) => {
      if (!action || typeof onRowAction !== "function") {
        return;
      }
      const actionId = typeof action === "string" ? action : action.id;
      onRowAction(actionId, displayRow, action);
    };
    ...
    return (
      <div className="dt__row-actions dt__row-actions--inline">
        {actions.map((action, index) => {
          ...
          return (
            <button
              key={`${normalized.id || normalized.label || "action"}-${index}`}
              type="button"
              className={resolveActionClassName(normalized.variant)}
              onClick={() => runAction(normalized)}
              disabled={Boolean(normalized.disabled)}
              title={normalized.title}
            >
              {normalized.label || normalized.id || "Action"}
            </button>
          );
        })}
      </div>
  ...
  const addActionText = String(addActionLabel || "Add").replace(/^\+\s*/, "");

  return (
    <div className={`dt${className ? ` ${className}` : ""}`}>
      {/* Toolbar */}
      {(downloadExcel || (filterMode === "manual" && hasSearchRow) || toolbarExtra) && (
        <div className="dt__toolbar">
          {filterMode === "manual" && hasSearchRow && (
            <button
              type="button"
              className="dt__btn"
      ...
      {removeConfirmItem && (
        <div className="dt__confirm-backdrop" role="dialog" aria-modal="true" aria-label="Confirm row removal" onClick={closeRemoveConfirm}>
          <div className="dt__confirm-card" onClick={(e) => e.stopPropagation()}>
            <h4 className="dt__confirm-title">Confirm Remove Action</h4>
            <p className="dt__confirm-message">{confirmRemoveMessage}</p>
            <div className="dt__confirm-actions">
              <button type="button" className="dt__confirm-btn dt__confirm-btn--secondary" onClick={closeRemoveConfirm}>
                Cancel
              </button>
              <button type="button" className="dt__confirm-btn dt__confirm-btn--danger" onClick={confirmRemove}>
                Remove
              </button>
            </div>
          </div>
                  ...
                  return (
                    ...
                    >
                      {selectionEnabled && col.key === "__dt_select" ? (
                        ...
                        <button type="button" className="dt__action-btn dt__action-btn--ghost dt__add-btn" onClick={handleAddRow}>
                          <ListPlusIcon className="dt__add-icon" />
                          {addActionText}
                        </button>
                      ) : col.key === "__dt_row_actions" ? (
                        <span className="dt__th-inner">{rowActionsColumnLabel}</span>
                      ) : (
            ...
            <tbody className="dt__tbody">
              {pageRows.length === 0 ? (
                ...
                pageRows.map((row, ri) => {
                  ...
                  const markRow = Boolean(editableEnabled && item && (item.isNew || item.isAdded || item.isEdited));
                  const markSelectionCell = markRow && selectionEnabled;
                  return (
                  <tr key={editableEnabled ? item.internalId : ri}>
                    {selectionEnabled && (
                      <td className={`dt__td dt__td--selection${markSelectionCell ? " dt__td--marker" : ""}`}>
                        <input
                          type="checkbox"
                          className="dt__selection-checkbox"
                    ...
                    {columns.map((col, colIndex) => (
                      <td key={col.key} className={`dt__td${markRow && !selectionEnabled && colIndex === 0 ? " dt__td--marker" : ""}`}
                    ...
                    {hasCustomRowActions && (
                      <td className="dt__td dt__td--actions">
                        {renderCustomActionCell(displayRow)}
                      </td>
                    )}
                    {editableEnabled && (
                      <td className="dt__td dt__td--actions">
