  ...
  const callLoadService = useCallback(
    async (index, snap) => {
      ...
      try {
        ...
        if (stage.autoSubmit) {
          // Pass freshly loaded data directly to avoid stale-prop races on immediate submit.
          setTimeout(() => formRefs.current[index]?.submit?.(data), 0);
        }
      } catch (err) {
        updateStage(index, {
    ...
    <div className="sfp" role="region" aria-label="Stage flow panel">
      ...
      <div className="sfp__content">
        {stages.map((stage, index) => {
          ...
          const allowEditInReadOnlyModes = Boolean(stage.componentProps?.allowEditInReadOnlyModes);
