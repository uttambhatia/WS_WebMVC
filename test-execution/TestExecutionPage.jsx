...
function TreeNode({ node, level, expandedSet, onToggleExpand, selectedSet, onToggleSelect, disabled }) {
      ...
      <div className="te__tree-node-row" style={{ paddingLeft: `${10 + level * 16}px` }}>
        ...
        <label className={`te__tree-label ${disabled ? "is-disabled" : ""}`}>
          <input
            ...
            disabled={disabled}
          />
          <span className="te__tree-icon" aria-hidden="true">{isDirectory ? "📁" : "📄"}</span>
    ...
    <div className="te__panel">
      <div className="te__panel-grid">
        <section className="te__tree-card">
          ...
          {!loadingTree && !treeError && treeNodes.length > 0 && (
            <div className="te__tree" role="tree" aria-label="Test script hierarchy">
              {treeNodes.map((node) => (
                <TreeNode
                  key={node.path}
                  node={node}
...
export default function TestExecutionPage() {
  ...
  return (
    <div className="te-page">
      <section className="te-page__main-pane">
