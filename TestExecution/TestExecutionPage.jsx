...
function parseScheduleInstant(value) {
  ...
  if (typeof value === "string") {
    ...
    const parsed = new Date(hasTimezoneSuffix ? normalized : `${normalized}Z`);
...
function formatDateTimeInTimeZone(value, timeZone) {
  const date = parseScheduleInstant(value);
  if (!date) return "-";
...
function formatDateTimeLocalInputInTimeZone(value, timeZone) {
  const date = parseScheduleInstant(value);
  if (!date) return "";
...
function localInputToUtcIso(value, timeZone) {
  if (!value) return "";

  const normalizedValue = value.length === 16 ? `${value}:00` : value;
  const parsed = new Date(`${normalizedValue}Z`);
  ...
  const zoneParts = getTimeZoneParts(new Date(guessUtcMs), timeZone);
  const zoneUtcMs = Date.UTC(
    Number(zoneParts.year),
    Number(zoneParts.month) - 1,
    Number(zoneParts.day),
    Number(zoneParts.hour),
    Number(zoneParts.minute),
    Number(zoneParts.second || 0)
  );
  const offsetMs = zoneUtcMs - guessUtcMs;
...
function normalizeExecution(row) {
  ...
  return {
    ...
    runType: String(row?.run_type || row?.runType || "ADHOC").toLowerCase(),
...
function normalizeSchedule(row) {
  const selectedScripts = normalizeSelectedScripts(row?.selected_scripts || row?.selectedScripts || []);
  return {
    scheduleId: row?.schedule_id || row?.scheduleId,
    runAt: row?.run_at || row?.runAt || null,
    timezoneName: row?.timezone_name || row?.timezoneName || row?.timezone_code || row?.timezoneCode || "-",
    timezoneCode: row?.timezone_code || row?.timezoneCode || null,
    frequencyName: row?.frequency_name || row?.frequencyName || (row?.recurring ? "Recurring" : "One-off"),
    scriptsSelectedCount: row?.scripts_selected_count ?? row?.scriptsSelectedCount ?? selectedScripts.length,
...
function TestExecutionPanel({
  ...
  useEffect(() => {
    ...
    if ((mode === "view" || mode === "rerun") && scheduleDetails) {
      ...
      const timezoneCode = scheduleDetails.timezone_code || scheduleDetails.timezoneCode || scheduleDetails.timezone?.timezone_code || scheduleDetails.timezone?.timezoneCode;
      ...
      if (runAt) {
        try {
          const localValue = formatDateTimeLocalInputInTimeZone(runAt, timezoneCode);
          console.log('[Schedule] Setting runAt input value to:', localValue);
          setScheduleRunAt(localValue);
        } catch (e) {
      ...
      if (timezone) {
        ...
        setSelectedTimezone({
          timezoneId: scheduleDetails.timezone_id || scheduleDetails.timezoneId,
          timezoneCode: timezoneCode || timezone,
...
export default function TestExecutionPage() {
  ...
  const [scheduleToggleConfirm, setScheduleToggleConfirm] = useState(null);
    ...
    const timer = window.setInterval(async () => {
      try {
        ...
        if (FINAL_STATUSES.has(normalized.status)) {
          const isScheduledExecution = Boolean(panelExecution?.scheduleId || normalized?.scheduleId);
  ...
  const openSchedulePanel = useCallback(async (mode, row) => {
    if (!row?.scheduleId) {
      return;
    }

    setPanelMode(mode);
    setPanelExecutionMode("schedule");
    setPanelRunError("");

    try {
      const details = await getScheduleDetails(row.scheduleId);
      const selectedScripts = normalizeSelectedScripts(details?.selected_scripts || row.selectedScripts || []);

      setScheduleDetails(details || row.raw || null);
      setPanelExecution({
        testId: details?.schedule_id ? `schedule-${details.schedule_id}` : "",
        status: details?.active ? "scheduled" : "inactive",
        selectedScripts,
        createdAt: null,
        startedAt: details?.run_at || row.runAt || null,
        finishedAt: null,
        elapsedSeconds: 0,
        executionItems: selectedScripts.map((script, index) => ({
          itemId: `schedule-${row.scheduleId}-${index}`,
          execScript: script,
          status: details?.active ? "scheduled" : "inactive",
          error: "",
          durationSeconds: 0,
      ...
      setScheduleDetails(row.raw || null);
      setPanelExecution({
        testId: `schedule-${row.scheduleId}`,
        status: row.active ? "scheduled" : "inactive",
        selectedScripts: row.selectedScripts || [],
        createdAt: null,
        startedAt: row.runAt || null,
        finishedAt: null,
        elapsedSeconds: 0,
        executionItems: (row.selectedScripts || []).map((script, index) => ({
          itemId: `schedule-${row.scheduleId}-${index}`,
          execScript: script,
          status: row.active ? "scheduled" : "inactive",
          error: "",
  ...
  const scheduleColumns = useMemo(
    () => [
      { key: "scheduleId", label: "Schedule Id", sortable: true },
      {
        key: "runAt",
        label: "Scheduled Date/Time",
        sortable: true,
        render: (value, row) => formatDateTimeInTimeZone(value, row?.timezoneCode),
      },
      { key: "timezoneName", label: "Time Zone" },
      { key: "frequencyName", label: "Frequency" },
      { key: "scriptsSelectedCount", label: "Scripts Selected", sortable: true },
      {
        key: "lastRunAt",
        label: "Last Run",
        sortable: true,
  ...
  const executionTableService = useCallback(async (params) => {
    ...
    setLiveExecutionIds(
      rows
        ...
        .filter((row) => !row.scheduleId && row.runType !== "scheduled")
        .map((row) => row.testId)
        .filter(Boolean)
    );
  ...
  const scheduleTableService = useCallback(async (params) => {
    ...
    const rows = extractRows(payload).map(normalizeSchedule);
    const total = extractTotal(payload, rows.length);
  ...
  const renderScheduleRowActions = useCallback(
    ({ row }) => (
      <div className="te__actions">
        <button
          type="button"
        ...
        <button
          type="button"
          className={`te__schedule-switch ${row.active ? "is-active" : ""}`.trim()}
          onClick={() => {
            setScheduleToggleConfirm({
          ...
          aria-label={`${row.active ? "Deactivate" : "Activate"} schedule ${row.scheduleId}`}
        >
          <span className="te__schedule-switch-track" aria-hidden="true">
            <span className="te__schedule-switch-thumb" />
          </span>
          <span className="te__schedule-switch-text">{row.active ? "Active" : "Inactive"}</span>
        </button>
        <button
          type="button"
  ...
  const closeScheduleToggleConfirm = useCallback(() => {
    setScheduleToggleConfirm(null);
  }, []);

  const confirmScheduleToggle = useCallback(async () => {
    if (!scheduleToggleConfirm) {
      return;
    }

    await updateScheduleStatus(scheduleToggleConfirm.scheduleId, { active: scheduleToggleConfirm.nextActive });
    setReloadToken((prev) => prev + 1);
    setScheduleToggleConfirm(null);
  }, [scheduleToggleConfirm]);

  const panelTitle =
    panelExecutionMode === "schedule"
      ? (panelMode === "rerun" ? "Edit Schedule" : "View Schedule")
      : panelMode === "new"
        ? "New Test Execution"
        : panelMode === "rerun"
      ...
      <div className="te-page__inline-panel-wrap">
          ...
          >
            <TestExecutionPanel
              ...
              onScheduleSaved={() => setReloadToken((prev) => prev + 1)}
            />
          </InlineFullscreenPanel>
        </div>
        ...
        {scheduleToggleConfirm && (
          <div
            className="te__confirm-backdrop"
            role="dialog"
            aria-modal="true"
            aria-label="Confirm schedule status update"
            onClick={closeScheduleToggleConfirm}
          >
            <div className="te__confirm-card" onClick={(event) => event.stopPropagation()}>
              <h4 className="te__confirm-title">Confirm Status Change</h4>
              <p className="te__confirm-message">
                {scheduleToggleConfirm.nextActive
                  ? `Activate schedule ${scheduleToggleConfirm.scheduleId}?`
                  : `Deactivate schedule ${scheduleToggleConfirm.scheduleId}?`}
              </p>
              <div className="te__confirm-actions">
                <button
                  type="button"
                  className="te__confirm-btn te__confirm-btn--secondary"
                  onClick={closeScheduleToggleConfirm}
                >
                  Cancel
                </button>
                <button
                  type="button"
                  className="te__confirm-btn te__confirm-btn--danger"
                  onClick={confirmScheduleToggle}
                >
                  Confirm
                </button>
              </div>
            ...
            {activeTab === "schedule" ? (
              <DataTable
                ...
                sortableColumns={[
                  "scheduleId",
                  "runAt",
                  "scriptsSelectedCount",
                  "lastRunAt",
                ]}
                searchableColumns={["scheduleId", "timezoneName", "frequencyName"]}
                serverSide
                pageSize={10}
                pageSizeOptions={[10, 20, 50]}
                service={scheduleTableService}
                buildServerQuery={buildServerQuery}
                reloadToken={reloadToken}
                renderRowActions={renderScheduleRowActions}
                rowActionsColumnWidth="24%"
              />
            ) : (
              <DataTable
                columns={columns}
